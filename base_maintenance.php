...
<?php
  ...
  else if ( $submit == "重建警告缓存" )
  {
     DropAlertCache($db);
     UpdateAlertCache($db);
  }
  else if ( $submit == "更新IP缓存" )
  {
     UpdateDNSCache($db);     
  }
  else if ( $submit == "重建IP缓存" )
  {
     DropDNSCache($db);
     UpdateDNSCache($db);
  }
  else if ( $submit == "更新Whois缓存" )
  {
     UpdateWhoisCache($db);     
  }
  else if ( $submit == "重建Whois缓存" )
  {
     DropWhoisCache($db);
     UpdateWhoisCache($db);
  }
  ...
  echo "<B>"._MNTDBTYPE."</B> $DBtype <BR>  
        ...
        <INPUT TYPE=\"submit\" NAME=\"submit\" VALUE=\"清空表\">
        <INPUT TYPE=\"submit\" NAME=\"submit\" VALUE=\"重建表\">";
  ...
  echo '<B>'._MNTAICTE.'</B> '.$event_cnt.'&nbsp&nbsp
        ...
        &nbsp;&nbsp;
        <INPUT TYPE="submit" NAME="submit" VALUE="更新警告缓存">
        &nbsp;&nbsp;
        <INPUT TYPE="submit" NAME="submit" VALUE="重建警告缓存">';
  ...
  echo '<B>'._MNTIPACUSIP.'</B> '.$uncached_sip_cnt.'&nbsp;&nbsp&nbsp;'.
       ...
       '<B>'._MNTIPACWC.'</B> '.$cached_dwhois_cnt.'<BR>
        <INPUT TYPE="submit" NAME="submit" VALUE="更新IP缓存">&nbsp;
        <INPUT TYPE="submit" NAME="submit" VALUE="更新Whois缓存"><BR>
        <INPUT TYPE="submit" NAME="submit" VALUE="重建IP缓存">&nbsp;
        <INPUT TYPE="submit" NAME="submit" VALUE="重建Whois缓存"><BR>';
